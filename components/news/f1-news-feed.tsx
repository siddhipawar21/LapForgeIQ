...
export default function F1NewsFeed() {
  ...
  const fetchNews = async () => {
    ...
    try {
      ...
      let json: NewsResponse;
      try {
        json = JSON.parse(text) as NewsResponse;
      } catch {
        console.error("Non-JSON response from /api/news/f1:", text);
        throw new Error("News service returned non-JSON response.");
      }
